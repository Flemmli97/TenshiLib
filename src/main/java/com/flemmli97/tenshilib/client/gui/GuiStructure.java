...
public class GuiStructure extends GuiScreen {
	...
	public void initGui()
    {
        ...
        this.buttonList.add(this.changePage=new GuiButton(0, this.width / 2 + 153 - (this.page==0?this.editWidth():this.backWidth()), this.page==0?80:40, this.page==0?this.editWidth():this.backWidth(), 20, this.page==0?I18n.format("structure_piece.edit"):I18n.format("structure_piece.back")));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
		...
		if(this.page==0)
		{
	        ...
	        this.drawString(this.fontRenderer, I18n.format("structure_piece.reinit"), this.width / 2 + 153 -this.fontRenderer.getStringWidth(I18n.format("structure_piece.reinit")), 30, 10526880);
	...
	protected void actionPerformed(GuiButton button) throws IOException
    {
		if(button==this.mirror)
		{
			...
			{
				case FRONT_BACK: this.tile.mirror(Mirror.NONE);
					break;
				case LEFT_RIGHT: this.tile.mirror(Mirror.FRONT_BACK);
					break;
				case NONE: this.tile.mirror(Mirror.LEFT_RIGHT);
					break;
			}
		...
		if(button==this.rot)
		{
			...
			{
				case CLOCKWISE_180: this.tile.rotate(Rotation.COUNTERCLOCKWISE_90);
					break;
				case CLOCKWISE_90: this.tile.rotate(Rotation.CLOCKWISE_180);
					break;
				case COUNTERCLOCKWISE_90: this.tile.rotate(Rotation.NONE);
					break;
				case NONE: this.tile.rotate(Rotation.CLOCKWISE_90);
					break;
			}
		...
		if(button==this.genType)
		{
			...
			{
				case FLOATING: this.tile.setGenerationType(GenerationType.REPLACEBELOW);
					break;
				case GROUNDALIGNED: this.tile.setGenerationType(GenerationType.FLOATING);
					break;
				case REPLACEBELOW: this.tile.setGenerationType(GenerationType.GROUNDALIGNED);
					break;
			}
		...
		if(button==this.save)
		{
			this.tile.setOffSet(new BlockPos(Integer.parseInt(this.offX.getText()),Integer.parseInt(this.offY.getText()),Integer.parseInt(this.offZ.getText())));
			...
			if(this.page==1)
			{
				...
				this.resources.forEach(triple->{
					if(!triple.getMiddle().getText().isEmpty())
						this.tile.addStructureName(Float.parseFloat(triple.getMiddle().getText()), triple.getLeft().getText().isEmpty()?null:new ResourceLocation(triple.getLeft().getText()));
				});
			}
			else
				this.tile.setOffSet(new BlockPos(Integer.parseInt(this.offX.getText()),Integer.parseInt(this.offY.getText()),Integer.parseInt(this.offZ.getText())));
	...
	public String mirrorString(Mirror mirror)
	{
		switch(mirror)
		{
			case FRONT_BACK:return "^ v";
			case LEFT_RIGHT:return "< >";
			case NONE:return "|";
		}
		return "";
	}
	...
	public String genString(GenerationType type)
	{
		switch(type)
		{
			...
			case GROUNDALIGNED: return "~~~";
			case REPLACEBELOW: return "___";
		}
		return "";
	}
}
