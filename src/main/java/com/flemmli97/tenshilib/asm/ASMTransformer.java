...
public class ASMTransformer implements IClassTransformer {
    ...
    /*
     * @SuppressWarnings("unused") private static void patchRenderLivingBase(ClassNode clss, MethodNode method) {
     * asmDebug("Patching RenderLivingBase.doRender"); boolean optifine=false; try {optifine =
     * Class.forName("optifine.OptiFineClassTransformer")!=null;} catch (ClassNotFoundException e) {}
     * Iterator<AbstractInsnNode> it = (Iterator<AbstractInsnNode>)method.instructions.iterator(); AbstractInsnNode node =
     * null; AbstractInsnNode setRotationAngles = null; Method setRotationAnglesMethod = new Method("setRotationAngles",
     * "func_78087_a", "a", "(FFFFFFLnet/minecraft/entity/Entity;)V", "(FFFFFFLvg;)V"); while (it.hasNext()) { node =
     * it.next(); if(node.getOpcode()==Opcodes.INVOKEVIRTUAL && node instanceof MethodInsnNode) { MethodInsnNode dyn =
     * (MethodInsnNode) node; //ModelBase.setRotationAngles if(setRotationAnglesMethod.matches(dyn)) setRotationAngles=dyn;
     * } } //shouldSit local variable missing with optifine. this seems so easily breakable... int i = optifine?1:0;
     * testNonNull(setRotationAngles, setRotationAnglesMethod); InsnList inject = new InsnList(); inject.add(new
     * VarInsnNode(Opcodes.FLOAD, 18-i)); inject.add(new VarInsnNode(Opcodes.FLOAD, 17-i)); inject.add(new
     * VarInsnNode(Opcodes.FLOAD, 15-i)); inject.add(new VarInsnNode(Opcodes.FLOAD, 13-i)); inject.add(new
     * VarInsnNode(Opcodes.FLOAD, 14-i)); inject.add(new VarInsnNode(Opcodes.FLOAD, 16-i)); inject.add(new
     * VarInsnNode(Opcodes.ALOAD, 1)); inject.add(new VarInsnNode(Opcodes.ALOAD, 0)); if(ASMLoader.isDeobfEnvironment())
     * inject.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "com/flemmli97/tenshilib/asm/ASMMethods", "modelEvent",
     * "(FFFFFFLnet/minecraft/entity/Entity;Lnet/minecraft/client/renderer/entity/RenderLivingBase;)V", false)); else
     * inject.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "com/flemmli97/tenshilib/asm/ASMMethods", "modelEvent",
     * "(FFFFFFLvg;Lcaa;)V", false)); method.instructions.insert(setRotationAngles, inject); }
     */

    /**
     * Add ModelPlayerRenderEvent in {@link net.minecraft.client.model.ModelPlayer#render} (Actually
     * {@link net.minecraft.client.model.ModelBiped#render} )
     */
    ...
    private static void layerHeldItem(ClassNode clss, MethodNode method) {
        ...
        //Right hand side
        /*
         * inject.add(new VarInsnNode(Opcodes.ALOAD, 1)); inject.add(new VarInsnNode(Opcodes.ALOAD, 11));
         * if(ASMLoader.isDeobfEnvironment()) { inject.add(new FieldInsnNode(Opcodes.GETSTATIC, "net/minecraft/util/EnumHand",
         * "MAIN_HAND", "Lnet/minecraft/util/EnumHand;")); inject.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
         * "com/flemmli97/tenshilib/asm/ASMMethods", "layerHeldItemEvent",
         ...
         * false)); inject.add(new VarInsnNode(Opcodes.ASTORE, 11)); } else { inject.add(new FieldInsnNode(Opcodes.GETSTATIC,
         * "ub", "a", "Lub;")); inject.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "com/flemmli97/tenshilib/asm/ASMMethods",
         * "layerHeldItemEvent", "(Lvp;Laip;Lub;)Laip;", false)); inject.add(new VarInsnNode(Opcodes.ASTORE, 11)); }
         */
        //Left hand side
        inject.add(new VarInsnNode(Opcodes.ALOAD, 1));
