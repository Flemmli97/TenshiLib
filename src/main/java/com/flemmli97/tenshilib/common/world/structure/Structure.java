...
public class Structure implements IConfigSerializable<Structure> {
     ...
     * Should be used for structures which are trackable and/or bigger than a chunk.
     * 
     * @param id                 The id of the whole structure. Translation key is "structure."+id.
     * @param startingStructures Structures structure pieces eligible for the first placement, if null the id will be
     *                           picked.
     ...
     */
    public Structure(ResourceLocation id, @Nullable ResourceLocation[] startingStructures, int frequency, int minDist, int maxParts,
            LocationType type, GenerationType genType, int[] dimID, int yOffset, boolean preventOtherMobSpawn) {
        this.id = id;
    ...
    public void startStructure(World world, int chunkX, int chunkZ, Random random) {
        ...
        if(this.frequency > 0 && rightDimension && random.nextInt(this.frequency) == 0
                && world.getBiomeProvider().areBiomesViable(chunkX, chunkZ, 0, this.biomes)){
            ...
            if((nearest != null && pos.distanceSq(nearest.getPos()) < (this.minDist * this.minDist))
                    || MinecraftForge.TERRAIN_GEN_BUS.post(new StructureGenerateEvent(this, pos, world)))
                return;
    ...
    public Structure config(Configuration config, String configCategory) {
        ...
        for(EnumCreatureType type : EnumCreatureType.values()){
            ...
            for(String s : config.getStringList(type.toString(), configCategory + ".spawns", def,
                    "Syntax is: <Full Classname>;<minGroup>;<maxGroup>;<weight>")){
    ...
    public static Biome.SpawnListEntry fromString(String s) {
        ...
        if(clss == null || !EntityLivingBase.class.isAssignableFrom(clss))
            return null;
        return new Biome.SpawnListEntry((Class<? extends EntityLiving>) clss, Integer.parseInt(subs[1]), Integer.parseInt(subs[2]),
