...
public class StructureLoader {
    ...
    public static Schematic getSchematic(String modid, String fileName) {
        try{
             ...
             * if(input!=null) return loadSchematic(input, world);
             */
    ...
    /*
     * private static Schematic loadSchematic(InputStream input, World world) { try { NBTTagCompound nbt =
     * CompressedStreamTools.readCompressed(input); short width = nbt.getShort("Width"); short height =
     * nbt.getShort("Height"); short length = nbt.getShort("Length"); byte[] blocks = nbt.getByteArray("Blocks"); byte[]
     * data = nbt.getByteArray("Data"); NBTTagList tileEntities = nbt.getTagList("TileEntities", 10); ArrayList<TileEntity>
     * tiles = new ArrayList<TileEntity>(); Map<Position, IBlockState> posBlockMapping = Maps.newHashMap(); NBTTagCompound
     * idNameMap = nbt.getCompoundTag("SchematicaMapping"); System.out.println(idNameMap); for (int x = 0; x < width; x++) {
     * for (int y = 0; y < height; y++) { for (int z = 0; z < length; z++) { final int index = x + (y * length + z) * width;
     * int blockID = blocks[index] & 0xFF; final int meta = data[index] & 0xFF; final Block block =
     * Block.REGISTRY.getObjectById(blockID); Position pos = new Position(x, y, z); try {
     * 
     * @SuppressWarnings("deprecation") final IBlockState blockState = block.getStateFromMeta(meta);
     * posBlockMapping.put(pos, blockState); } catch (final Exception e) {
     * RuneCraftory.logger.error("Error with block state at {"+pos+"} with block {"+block+"} and metadata {"+meta+"}"); } }
     * } }
     * 
     * for(int i = 0; i < tileEntities.tagCount(); i++) { tiles.add(TileEntity.create(world,
     * tileEntities.getCompoundTagAt(i))); } Schematic schematic = new Schematic(width, height, length); return schematic; }
     * catch (IOException e) { RuneCraftory.logger.error("Error reading schematic file", e); return null; } }
     */
