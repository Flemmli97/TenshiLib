...
public class RayTraceUtils {
    ...
    public static List<EntityLivingBase> getEntities(EntityLivingBase entity, float reach, float aoe) {
        return getEntitiesIn(EntityLivingBase.class, entity, entity.getPositionVector().addVector(0, entity.height / 2, 0), entity.getLook(1), reach,
                aoe, null);
    }

    public static <T extends EntityLivingBase> List<T> getEntitiesIn(Class<T> clss, EntityLivingBase entity, Vec3d pos, Vec3d look, float reach,
        ...
        return entity.world.getEntitiesWithinAABB(clss, entity.getEntityBoundingBox().grow(reach),
                (living) -> living != entity && (pred == null || pred.apply(living)) && !living.isOnSameTeam(entity) && living.canBeCollidedWith()
                        && circ.intersects(living.world, living.getEntityBoundingBox()));
    }
    ...
    public static RayTraceResult calculateEntityFromLook(EntityLivingBase entity, Vec3d pos, Vec3d dir, float reach, Class<? extends Entity> clss,
        ...
        List<Entity> list = entity.world.getEntitiesWithinAABB(clss,
                entity.getEntityBoundingBox().expand(dir.x * reach, dir.y * reach, dir.z * reach).expand(1.0D, 1.0D, 1.0D),
                (t) -> EntitySelectors.NOT_SPECTATING.apply(t) && t != null && t != entity && t.canBeCollidedWith()
                        && (pred == null || pred.apply(t)));
        for(Entity e : list){
    ...
    public static BlockPos randomPosAround(World world, Entity e, BlockPos pos, int range, boolean grounded, Random rand) {
        ...
        while(pos1.getY() < 255 && (!world.getBlockState(pos1.down()).isSideSolid(world, pos1.down(), EnumFacing.UP)
                || world.collidesWithAnyBlock(e.getEntityBoundingBox().offset(pos1)))){
            pos1 = pos1.up();
        }
        return pos1;
    }
    ...
    public static RayTraceResult entityRayTrace(Entity e, float range, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox,
            boolean returnLastUncollidableBlock, boolean includeEntities, @Nullable Predicate<? super Entity> pred) {
        Vec3d posEye = e.getPositionEyes(1);
        Vec3d look = posEye.add(e.getLookVec().scale(range));
        if(includeEntities){
            Entity entity = null;
            double d0 = 0;
            List<Entity> list = e.world.getEntitiesInAABBexcluding(e,
                    new AxisAlignedBB(e.posX, e.posY, e.posZ, e.posX + look.x, e.posY + look.y, e.posZ + look.z).grow(1),
                    pred != null ? pred : EntitySelectors.NOT_SPECTATING);
        ...
        return blockpos != null ? blockpos
                : new RayTraceResult(RayTraceResult.Type.MISS, look, EnumFacing.getFacingFromVector((float) look.x, (float) look.y, (float) look.z),
                        null);
    }
