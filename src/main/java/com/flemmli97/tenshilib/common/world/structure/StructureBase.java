...
public class StructureBase {
    ...
    public StructureBase(ResourceLocation id, ResourceLocation schematicID, Random random, BlockPos pos, Rotation rot, Mirror mirror,
            GenerationType genType, int maxSize) {
    ...
    protected void addParts(Schematic schematic, BlockPos pos, Random random) {
        schematic.getTileEntities().forEach(entry -> {
            if(entry.getValue().getString("id").equals(structureTile.toString())){
                ...
                BlockPos place = Schematic.transformPos(entry.getKey(), this.mirror, this.rot, new BlockPos(schematic.x, schematic.y, schematic.z))
                        .add(pos);
    ...
    public static StructureBoundingBox getBox(Schematic schem, Rotation rot, BlockPos pos) {
        ...
        return StructureBoundingBox.createProper(corner.getX() + pos.getX(), corner.getY() + pos.getY(), corner.getZ() + pos.getZ(), pos.getX(),
                pos.getY(), pos.getZ());
    }
