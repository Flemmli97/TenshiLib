...
public class ItemUtil {
	...
	{
		double dmg=0;
		Collection<AttributeModifier> atts = stack.getAttributeModifiers(EntityEquipmentSlot.MAINHAND).get(SharedMonsterAttributes.ATTACK_DAMAGE.getName());
	...
	public static void initItemLists()
	{
		ForgeRegistries.ITEMS.forEach(entry ->{
			if(entry instanceof ItemArmor)
			{
				...
				{
					case CHEST:chest.add(entry);
						break;
					case FEET:boots.add(entry);
						break;
					case HEAD:helmet.add(entry);
						break;
					case LEGS:legs.add(entry);
						break;
				}
			}
	...
	public static List<Item> getList(EntityEquipmentSlot slot)
	{
		switch(slot)
		{
			case CHEST: return ImmutableList.copyOf(chest);
			case FEET: return ImmutableList.copyOf(boots);
			case HEAD: return ImmutableList.copyOf(helmet);
			case LEGS: return ImmutableList.copyOf(legs);
			case MAINHAND: return ImmutableList.copyOf(weapons);
			case OFFHAND: return ImmutableList.copyOf(tools);
		}
	...
	public static ItemStack getRandomFromSlot(EntityEquipmentSlot slot)
	{
		...
		switch(slot)
		{
			...
			case MAINHAND:return new ItemStack(weapons.get(rand.nextInt(weapons.size())));
			default:
				break;
		}
