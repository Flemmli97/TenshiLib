...
public class CircleSector {
    ...
    public CircleSector(Vec3d pos, Vec3d look, float radius, float theta) {
        ...
        this.radius=radius;
        this.rotAxis=MathUtils.rotate(MathUtils.rotate(new Vec3d(0,1,0),new Vec3d(1,0,0),(float) MathHelper.atan2(look.x, look.z)), look, -MathUtils.degToRad(90)).normalize();
        this.center=pos;
    ...
    public boolean intersects(World world, AxisAlignedBB aabb) {
        ...
        while(!flag && rot <= this.theta) {
            RayTraceResult blocks = world.rayTraceBlocks(this.center, this.center.addVector(ray.x * this.radius, ray.y * this.radius, ray.z * this.radius), false, false, true);
