...
public class JsonModelHelper {
    ...
    public static void itemJson(File assetsFolder, Iterable<Item> list) {
        try {
            for (Item item : list) {
                File jsonFile = new File(assetsFolder, item.getRegistryName().getResourcePath() + ".json");
                if (!jsonFile.exists()) {
    ...
    public static void blockJson(File assetsFolder, Iterable<Block> list, @Nullable Function<Block, String> parentModel) {
        try {
            ...
            for (Block block : list) {
                ...
                File jsonFile2 = new File(assetsFolder+"/model", block.getRegistryName().getResourcePath() + ".json");
                if (!jsonFile2.exists()) {
                ...
                File jsonFile3 = new File(assetsFolder+"/item", block.getRegistryName().getResourcePath() + ".json");
                if (!jsonFile3.exists()) {
    ...
    public static void lang(File assetsFolder, List<Item> list) {
        try {
            ...
            if (!jsonFile.exists()) {
                ...
                for (Item item : list) {
                    buf.write(item.getUnlocalizedName() + ".name=");
                    buf.newLine();
                }
                buf.close();
            }
