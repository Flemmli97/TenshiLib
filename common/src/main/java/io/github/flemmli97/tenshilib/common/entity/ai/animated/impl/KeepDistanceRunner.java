...
public class KeepDistanceRunner<T extends PathfinderMob & IAnimated> implements ActionRun<T> {
    ...
    private final double minDistSqr, maxDistSqr, speed;
    private final int dist;

    private boolean start, moveCloser;

    public KeepDistanceRunner(double minDist, double maxDist, double speed, int dist) {
    ...
    public boolean run(AnimatedAttackGoal<T> goal, LivingEntity target, AnimatedAction anim) {
        ...
        return this.moveCloser ? goal.distanceToTargetSq < this.maxDistSqr : goal.attacker.getNavigation().isDone();
    }
}
