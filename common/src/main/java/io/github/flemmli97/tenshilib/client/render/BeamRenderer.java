...
public abstract class BeamRenderer<T extends BeamEntity> extends EntityRenderer<T> {
    ...
    public BeamRenderer(EntityRendererProvider.Context ctx, float width) {
        this(ctx, width, 4);
    }

    public BeamRenderer(EntityRendererProvider.Context ctx, float width, int polygon) {
        this(ctx, width, width - 0.25f, polygon);
    }

    public BeamRenderer(EntityRendererProvider.Context ctx, float glowWidth, float innerWidth, int polygon) {
        super(ctx);
    ...
    protected void renderBeam(T entity, float dist, float width, PoseStack poseStack, MultiBufferSource buffer, int packedLight, boolean playerView) {
        ...
        if (this.startTexture(entity) != null) {
            ...
            if (playerView) {
                this.renderBeam(poseStack, buffer.getBuffer(this.getRenderLayer(entity, this.startTexture(entity).res())), 0, startLength, width, -width, this.currentAnimation(entity, BeamPart.START), this.animationFrames(BeamPart.START), packedLight, true);
            } else {
                this.render3dBeam(poseStack, buffer.getBuffer(this.getRenderLayer(entity, this.startTexture(entity).res())), this.points, 0, startLength, this.currentAnimation(entity, BeamRenderer.BeamPart.MIDDLE), (float) this.animationFrames(BeamRenderer.BeamPart.MIDDLE), packedLight);
                alphaPre = this.alpha;
                this.setAlpha(this.glowAlpha(entity));
                this.render3dBeam(poseStack, buffer.getBuffer(this.getGlowingRenderLayer(entity, this.startTexture(entity).res())), this.pointsGlow, 0, startLength, this.currentAnimation(entity, BeamRenderer.BeamPart.MIDDLE), (float) this.animationFrames(BeamRenderer.BeamPart.MIDDLE), packedLight);
        ...
        if (this.endTexture(entity) != null) {
            ...
            length -= endLength;
            if (playerView) {
                this.renderBeam(poseStack, buffer.getBuffer(this.getRenderLayer(entity, this.endTexture(entity).res())), startLength + length, endLength, width, -width, this.currentAnimation(entity, BeamPart.END), this.animationFrames(BeamPart.END), packedLight, true);
            } else {
                this.render3dBeam(poseStack, buffer.getBuffer(this.getRenderLayer(entity, this.startTexture(entity).res())), this.points, startLength + length, endLength, this.currentAnimation(entity, BeamRenderer.BeamPart.MIDDLE), (float) this.animationFrames(BeamRenderer.BeamPart.MIDDLE), packedLight);
                alphaPre = this.alpha;
                this.setAlpha(this.glowAlpha(entity));
                this.render3dBeam(poseStack, buffer.getBuffer(this.getGlowingRenderLayer(entity, this.startTexture(entity).res())), this.pointsGlow, startLength + length, endLength, this.currentAnimation(entity, BeamRenderer.BeamPart.MIDDLE), (float) this.animationFrames(BeamRenderer.BeamPart.MIDDLE), packedLight);
                this.setAlpha(alphaPre);
            }
        }
        ...
        for (int d = 0; d < segments.length; d++) {
            if (playerView) {
                this.renderBeam(poseStack, builder, startLength + (float) d * this.segmentLength(), segments[d], width, -width, this.currentAnimation(entity, BeamRenderer.BeamPart.MIDDLE), this.animationFrames(BeamRenderer.BeamPart.MIDDLE), packedLight, true);
            } else {
                this.render3dBeam(poseStack, builder, this.points, startLength + (float) d * this.segmentLength(), segments[d], this.currentAnimation(entity, BeamRenderer.BeamPart.MIDDLE), this.animationFrames(BeamRenderer.BeamPart.MIDDLE), packedLight);
            }
        }
        if (!playerView) {
            alphaPre = this.alpha;
            ...
            for (int d = 0; d < segments.length; d++) {
                this.render3dBeam(poseStack, builder, this.pointsGlow, startLength + (float) d * this.segmentLength(), segments[d], this.currentAnimation(entity, BeamRenderer.BeamPart.MIDDLE), (float) this.animationFrames(BeamRenderer.BeamPart.MIDDLE), packedLight);
            }
            this.setAlpha(alphaPre);
        }
    }
