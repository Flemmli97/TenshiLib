...
public class ActionUtils {
    ...
    public static <T extends PathfinderMob & AnimatedEntity> GoalAttackAction.Condition<T> ranged(double range) {
        return (goal, target, previousAnim) -> goal.distanceToTargetSq < range * range;
    }

    public static <T extends PathfinderMob & AnimatedEntity> GoalAttackAction.Condition<T> rangedVisible(double range) {
     ...
     */
    public static <T extends PathfinderMob & AnimatedEntity> GoalAttackAction<T> failedStart() {
        return new GoalAttackAction<>(null);
    }

    public static <T extends PathfinderMob & AnimatedEntity> GoalAttackAction.Condition<T> chanced(FloatGetter<T> chance) {
        return (goal, target, previousAnim) -> goal.attacker.getRandom().nextFloat() < chance.get(goal.attacker);
    }

    public static <T extends PathfinderMob & AnimatedEntity> GoalAttackAction.Condition<T> chanced(FloatGetter<T> chance, GoalAttackAction.Condition<T> other) {
