...
public interface SelectableEntry {
    ...
    static FormattedCharSequence dottedWith(Font font, Component text, int maxWidth, boolean noStyle) {
        ...
        if (w > maxWidth) {
            ...
            return FormattedCharSequence.composite(Language.getInstance().getVisualOrder(sub), new TextComponent("...").withStyle(text.getStyle()).getVisualOrderText());
        }
    ...
    void render(SelectableListWidget widget, PoseStack poseStack, int mouseX, int mouseY, float partialTick, int x, int y, boolean selected, boolean hovered);
