...
/**
 * An {@link EditBox} that can suggest values when typing.
 */
public class SuggestionEditBox extends EditBox {
    ...
    public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        GuiComponent.fill(poseStack, this.rect.getX(), this.rect.getY(), this.rect.getX() + this.rect.getWidth(), this.rect.getY() + this.rect.getHeight(), 0xe0101010);
        int x = this.x + this.paddingX;
        ...
        for (int i = 0; i < this.suggestions.length; i++) {
            ...
            this.font.drawShadow(poseStack, string, x, y + i * this.lineHeight, this.current == idxx ? 0xFFFF55 : 0xFFFFFF);
        }
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (this.rect.contains((int) mouseX, (int) mouseY)) {
            this.offset = Mth.clamp((int) (this.offset - delta), 0, Math.max(this.suggestions.length - this.limit, 0));
            return true;
        }
        return false;
    }
