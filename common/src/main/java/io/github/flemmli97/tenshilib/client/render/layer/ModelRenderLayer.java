...
public class ModelRenderLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLightIn, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (rendertype != null) {
            ...
            this.model.renderToBuffer(poseStack, ivertexbuilder, packedLightIn, i, FastColor.ARGB32.colorFromFloat(flag1 ? 0.15F : 1.0F, 1, 1, 1));
        }
        poseStack.popPose();
    }
